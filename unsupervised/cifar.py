...
def train(net, optimizer, trainloader, criterion, lemniscate, epoch):
    ...
    for batch_idx, (inputs, targets, indexes) in enumerate(trainloader):
        ...
        inputs, targets, indexes = inputs.to(args.device), targets.to(args.device), indexes.to(args.device)
        optimizer.zero_grad()
        ...
        if batch_idx % 100 == 0:
            print('Epoch: [{}/{}][{}/{}] '
                ...
                epoch, args.epochs, batch_idx, len(trainloader),
...
def get_data_loader():
    ...
    if args.transform_crop == 'RandomResizedCrop':
        crop = transforms.RandomResizedCrop(size=32, scale=(args.transform_scale, 1.))
    else:
    ...
    trainset = datasets.CIFAR10Instance(root=args.data_dir, train=True, download=True, transform=transform_train)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=128, shuffle=True, num_workers=2)

    testset = datasets.CIFAR10Instance(root=args.data_dir, train=False, download=True, transform=transform_test)
...
def build_model():
    ...
    if args.architecture == 'resnet18':
        ...
        net = models.WideResNet(depth=28, num_classes=args.low_dim, widen_factor=2, dropRate=0).to(args.device)
    elif args.architecture == 'wrn-28-10':
        net = models.WideResNet(depth=28, num_classes=args.low_dim, widen_factor=10, dropRate=0).to(args.device)

    # define leminiscate
    if args.nce_k > 0:
        lemniscate = NCEAverage(args.low_dim, args.ndata, args.nce_k, args.nce_t, args.nce_m)
    else:
        lemniscate = LinearAverage(args.low_dim, args.ndata, args.nce_t, args.nce_m)
    ...
    if args.lr_scheduler == 'multi-step':
        if args.epochs == 200:
            ...
            raise RuntimeError(f"need to config steps for epoch = {args.epochs} first.")
        scheduler = lr_scheduler.MultiStepLR(optimizer, steps, gamma=0.2, last_epoch=start_epoch - 1)
    elif args.lr_scheduler == 'cosine':
        scheduler = lr_scheduler.CosineAnnealingLR(optimizer, args.epochs, eta_min=0.00001, last_epoch=start_epoch - 1)
    elif args.lr_scheduler == 'cosine-with-restart':
        scheduler = CosineAnnealingLRWithRestart(optimizer, eta_min=0.00001, last_epoch=start_epoch - 1)
    else:
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if acc > best_acc:
            ...
            torch.save(state, os.path.join(args.model_dir, 'ckpt.cifar.pth.tar'))
            best_acc = acc
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-dir', '--dataDir', default='./data', type=str, metavar='DIR')
